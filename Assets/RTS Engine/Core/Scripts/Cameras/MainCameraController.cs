...
namespace RTSEngine.Cameras
{
    public class MainCameraController : MonoBehaviour, IMainCameraController
    {
        ...
        [SerializeField, Tooltip("Populate this array by one or more terrain area types to offset the height of the camera by the height of the terrain area objects when the camera is facing them.")]
        ...
        public void Init(IGameManager gameMgr)
        {
            ...
            {
                logger.LogError($"[{GetType().Name}] The field 'Height Offset Terrain Areas' must be empty or include valid elements!", source: this);
                return;
            }
        ...
        private void Zoom()
        {
            ...
            else
            {
                ...
                if (terrainMgr.ScreenPointToTerrainPoint(RTSHelper.ScreenMiddlePoint, heightOffsetTerrainAreas, out Vector3 terrainPoint)
                    && terrainPoint.y >= offsetMinTerrainHeight)
                    targetHeight += terrainPoint.y - offsetMinTerrainHeight;
