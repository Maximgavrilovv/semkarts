...
namespace RTSEngine.Task
{
    ...
    public class EntityTasksQueueHandler : EntityComponentBase, IEntityTasksQueueHandler
    {
        ...
        private ErrorMessage AddLocal(SetTargetInputData input, bool launchOnEmpty = true)
        {
            ...
            {
                if ((!keepActiveTask || IsCollectingResource()) || Entity.IsIdle) 
        ...
        private bool IsCollectingResource() => resourceCollector.IsValid() && resourceCollector.HasTarget && excludeResourceCollectionInKeepActiveTask;
        ...
        private bool TryLaunchNext(bool directLaunch = false)
        {
            ...
            if (Entity.EntityTargetComponents[nextInput.componentCode].IsTargetValid(nextInput.target, nextInput.playerCommand) != ErrorMessage.none)
                return false;
