...
namespace RTSEngine.BuildingExtension
{
    public class BuildingPlacer : MonoBehaviour, IBuildingPlacer, IEntityPreInitializable
    {
        ...
        {
            if (data.targetSegmentIndex == data.sourceSegmentIndex)
            {
                CanPlace = false;
        ...
        private void OnDebugPlacement()
        {
            ...
            string debugMessage = $"[{Building.Code}] Border: {IsBuildingInBorder()} - Map Terrain: {IsBuildingOnMap()} - Overlapped Colliders: {string.Join(" - ", overlappedColliders.Where(collider => collider != null))} - Conditions: {(!Conditions.Any() || Conditions.All(condition => condition.CanPlaceBuilding(Building)))} - Building Placement Condition: {placerMgr.CanPlace(this)}";
