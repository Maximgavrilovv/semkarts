            ...
            if (errorMsg != ErrorMessage.none)
            {
                switch (errorMsg)
                {
                    case ErrorMessage.placementBuildingCenterMissing:
                        logger.LogError($"[{GetType().Name} - {factionMgr.FactionID}] Building Center for building prefab '{task.Prefab.Code}' hasn't been specified in the Building Placement Request!");
                        break;
                ...
                LogEvent($"'{task.Prefab.Code}': Place Request Failure - Creation Tasks Requirements Not Met - Erorr: {errorMsg}");
                return false;
            }

            LogEvent($"'{task.Prefab.Code}': Place Request Success");
